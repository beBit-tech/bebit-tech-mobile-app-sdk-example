...
const PaymentScreen = ({navigation, route}: any) => {
  ...
  const buttonPressHandler = () => {
    ...
    // Wiki: https://github.com/beBit-tech/bebit-tech-react-native-app-sdk/wiki/Send-Action-(Event)-Examples#custom-event
    // Purpose: Track custom events specific to your business needs
    const products: [OSGProduct] = CartList.map((item: any) => ({id: item.id, name: item.name}));
    const event = OSGEventBuilder.purchase(`transactionId-${route.params.amount}`,route.params.amount, products)
          ...
          {PaymentList.map((data: {name: string, icon: string, isIcon: boolean}) => (
            <TouchableOpacity
              ...
              onPress={() => {
                ...
                // Wiki: https://github.com/beBit-tech/bebit-tech-react-native-app-sdk/wiki/Send-Action-(Event)-Examples#custom-event
                // Purpose: Track custom events specific to your business needs
                OmniSegment.trackEvent(OSGEventBuilder.custom(
                  "PaymentMode",
                  JSON.stringify({
                    name: data.name,
                    amount: 123456,
                ...
                OmniSegment.trackEvent(OSGEventBuilder.custom(
                  "date",
                  "2023-06-07T06:09:29.167342+00:00"
                ));
              }}>
              <PaymentMethod
